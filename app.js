...
window.onload = () => {
  ...
  //  генерируем препятствия 
    ...
    gameOverScreenPopup(){
      ...
      if (this.scoreElement.textContent > this.hiscore) {
        ...
        hiscoreSelector.textContent = `лучший счет: ${this.hiscore}`
      }
      window.innerWidth > 500 ? restartMessageSelector.textContent = 'пробел для рестарта' : restartMessageSelector.textContent = 'тапнуть'
    ...
    clearGame(event){
      if (event.key === ' ' || event.type === "click") {
        ...
        gameOverScoreSelector.textContent = 'счёт: ';  
  ...
  //  обнуляем все таймеры при смерти, возвращаем в исходное положение
    resetTimers(){
      this.birdSpeed = 0;
